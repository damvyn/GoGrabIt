...
Describe 'New-ImageGlass.ps1' {
    BeforeAll {
        $script:RootPath = Split-Path -Path $PSScriptRoot -Parent
        $script:ScriptDir = Join-Path -Path $RootPath -ChildPath "Scripts"
        $script:ScriptPath = Join-Path -Path $ScriptDir -ChildPath "New-ImageGlass.ps1"

        $script:FakeReply = [pscustomobject]@{
            tag_name = 'v8.5.0'
            assets = @(
                [pscustomobject]@{
                    name = 'ImageGlass_8.5.0_win-x64.msi'
                    browser_download_url = 'https://example.com/download/ImageGlass_8.5.0_win-x64.msi'
                },
                [pscustomobject]@{
                    name = 'ImageGlass_8.5.0_win-x86.msi'
                    browser_download_url = 'https://example.com/download/ImageGlass_8.5.0_win-x86.msi'
                },
                [pscustomobject]@{
                    name = 'ImageGlass_8.5.0.zip'
                    browser_download_url = 'https://example.com/download/ImageGlass_8.5.0.zip'
                }
            )
        }
    }
    ...
    Context 'Happy Path' {
        ...
        It 'Call Invoke-RestMethod for correct API' {
            Should -Invoke Invoke-RestMethod -Times 1 -Exactly -ParameterFilter {
                $Uri -eq 'https://api.github.com/repos/d2phap/ImageGlass/releases/latest'
            }
        }

        It 'Search pattern *win-x64.msi select correct asset' {
            Should -Invoke Invoke-WebRequest -Times 1 -Exactly -ParameterFilter {
                $Uri -eq 'https://example.com/download/ImageGlass_8.5.0_win-x64.msi'
            }
        }

        It 'Create folder structure Vendor\Product-Version' {
            $expectedDir = Join-Path $TestDir 'DuingDieuPhap\ImageGlass-8.5.0'
            $expectedDir | Should -Exist
        }

        It 'Download msi to the Vendor\Product-Version' {
            $expectedFile = Join-Path $TestDir 'DuingDieuPhap\ImageGlass-8.5.0\ImageGlass_8.5.0_win-x64.msi'
            $expectedFile | Should -Exist
        }

        It 'Create Install.ps1 script' {
            $expectedInstallScript = Join-Path $TestDir 'DuingDieuPhap\ImageGlass-8.5.0\Install.ps1'
            $expectedInstallScript | Should -Exist
        }

        It 'Install.ps1 contains correct values' {
            $installScript = Join-Path $TestDir 'DuingDieuPhap\ImageGlass-8.5.0\Install.ps1'
            $content = Get-Content -Path $installScript -Raw
            $content | Should -Match ([regex]::Escape('ImageGlass_8.5.0_win-x64.msi'))
    ...
    Context 'Call without -Destination' {
        ...
        It 'Use $PSScriptRoot as default destination root' {
            ...
            $expectedDir = Join-Path $ScriptDir 'DuingDieuPhap\ImageGlass-8.5.0'
            ...
            try {
                ...
                Remove-Item -Path (Join-Path $ScriptDir 'DuingDieuPhap') -Recurse -Force -ErrorAction SilentlyContinue
            }
        }
    }
